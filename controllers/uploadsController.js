...
const fileUpload = (req, res = response) => {
    ...
    const coleccionesValidas = ['actores', 'peliculas', 'ediciones', 'presupuestos', 'usuarios'];
    if (!coleccionesValidas.includes(coleccion)) {
        return res.status(400).json({
            ok: false,
            msg: 'No es una coleccion valida (usuarios, actores, peliculas, ediciones, presupuestos)'
        });
    }
